...
export function WatchList() {
  ...
  return (
    <div className="flex flex-wrap">
      {state.watchsLoading ? <Spinner className="dark:text-white" /> : null}
      {error ? (
      ...
      {state.watchs.map((item, idx) => {
        const date = new Date(item.created_at as string);

        const checked = !!state.cancelWatchs?.[item.id];
        const disabled = !!state.canceledWatchs?.[item.id];
        if (checked && !disabled) {
          total += 1;
        }
        return (
          <div
            ...
            <h3 className="font-semibold flex gap-2 items-center flex-wrap">
              <Checkbox
                ...
                isDisabled={disabled}
                onChange={() => {
                  actions.selectUnwatchRepo(item.id);
                }}
              />{' '}
          ...
          <Popover>
            <PopoverTrigger>
              <Button
                colorScheme="red"
                size={'lg'}
                isLoading={state.cancelWatchsLoading}
                isDisabled={state.cancelWatchsLoading}>
                {m.confirm()} ({total})
              </Button>
            </PopoverTrigger>
            <PopoverContent>
              <PopoverArrow />
              <PopoverCloseButton />
              <PopoverHeader>{m.confirm_cancel_watchs()}</PopoverHeader>
              <PopoverBody>
                {!state.cancelWatchsLoading && (
                  <Button
                    onClick={actions.cancelWatchs}
                    colorScheme="red"
                    className="w-full"
                    variant={'outline'}>
                    {m.confirm()}
